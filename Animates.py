...
class Animate:
    ...
    def attack(self):
        ...
        self.target.takeDamage(
            random.randint(
                self.weapon.getDamage() - 2, self.weapon.getDamage() + 2
            )
        )
    ...
    def takeDamage(self, damagePoints):
        ...
        if self.isDefending():
            myDamage = int(
                (damagePoints * (1 - (self.armourProtection / 200)))
                / random.randint(2, 4)
            )
...
class Player(Animate):
    ...
    def incrementXP(self):
        ...
        print(
            "You need another "
            + str(self.levelXPs[self.level] - self.xp)
            + " XP to level up"
        )
    ...
    def search(self, hcs):
        if not self.attacking:
            if hcs.getItemsInRoom(self.currentRoom, "list") != [[], [], []]:
                ...
                print(
                    "Items in this room: "
                    + hcs.getItemsInRoom(self.currentRoom, "str")
                )
        ...
        if hcs.getEnemiesInRoom(self.currentRoom, "str") != "":
            ...
            while not type(target) is int:
                try:
                    ...
                    print("This input must be a number")
            self.makeTarget(
                hcs.getEnemiesInRoom(self.currentRoom, "list")[target - 1]
            )
            self.attacking = True
    ...
    def explore(self, hcs):
        if not self.attacking:
            ...
            while choosing:
                ...
                if (
                    hcs.findNewRoom(
                        self.currentRoom, directionInt, "str", "test"
                    )
                    ...
                    print(
                        hcs.findNewRoom(
                            self.currentRoom, directionInt, "str", "non"
                        )
                    )
                else:
                    print(
            ...
            if (
                hcs.findNewRoom(self.currentRoom, directionInt, "num", "test")
                != 0
            ):
                self.currentRoom = int(
                    hcs.findNewRoom(
                        self.currentRoom, directionInt, "num", "non"
                    )
                )
        ...
        if hcs.getPeopleInRoom(self.currentRoom, "str") != "":
            ...
            while not type(target) is int:
                try:
                    ...
                    print("This input must be a number")
            if target != 0:
                hcs.getPeopleInRoom(self.currentRoom, "list")[
                    target - 1
                ].doDialogue()
        ...
        if self.backpack != []:
            ...
            while not type(target) is int:
                try:
                    ...
                    print("This input must be a number")
            if target == 1:
                self.backpack.remove(chosenItem)
            if target == 2:
